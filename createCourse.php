...
if ($inputSubmit == "Créer un cours") {
    if ($_FILES["img"]["name"] != "" && $inputShortDescription && $inputCourseName != "" && $inputIdLangue !== null && $inputIdTheme != null && $inputDescription != "" && $inputIdDifficulties != null) {
        ...
        if ($uploadValid == 0) {
            echo "Désolé, votre fichier n'a pas été importer.";
        } else {
            if (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
                echo "Le fichier " . htmlspecialchars(basename($_FILES["img"]["name"])) . " a été importé.";
            } else {
                echo "Désolé, il y avait une erreur durant l'importation de votre fichier.";
            }
        }
            ...
            <form method="post" name="create_cours_form" onsubmit="ValidateCreateCoursForm()" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <select class="form-control" name="langue" id="langue">
                        <?php foreach ($lstLanguages as $language) {
                            $selected = "";
                            if (isset($inputIdLangue)) {
                                if ($language["idLanguage"] == $inputIdLangue) {
                                    $selected = "selected"; // Sticky form
                                }
                            } ?>
                            <option style="color: black;" value="<?= $language["idLanguage"] ?>" name="language">
                ...
                <div class="form-group">
                    <select class="form-control" name="theme" id="theme">
                        <?php foreach ($lstThemes as $theme) {
                            $selected = "";
                            if (isset($inputIdTheme)) {
                                if ($theme["idTheme"] == $inputIdTheme) {
                                    $selected = "selected"; // Sticky form
                                }
                            } ?>

                            <option style="color: black;" value="<?= $theme["idTheme"] ?>" name="theme" <?= $selected ?>>
                                <?= $theme["name"] ?></option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <select class="form-control" name="difficulties" id="difficulties">
                        <?php foreach ($lstDifficulties as $difficultie) {
                            $selected = "";
                            if (isset($inputIdDifficulties)) {
                                if ($difficultie["idDifficulty"] == $inputIdDifficulties) {
                                    $selected = "selected";
                                }
                            } ?>
                            <option style="color: black;" value="<?= $difficultie["idDifficulty"] ?>" name="difficulty">
                                <?= $difficultie["name"] ?></option>
                        <?php } ?>
                    </select>
                </div>
